...
export async function POST(request: NextRequest) {
  try {
    ...
    let results;
    if (queryEmbedding && allTools.some((t) => t.embedding)) {
      results = allTools
...
async function generateQueryEmbedding(query: string): Promise<number[] | null> {
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a text encoder. Output exactly 64 comma-separated floating point numbers between -1 and 1 representing the semantic meaning. Output ONLY numbers, nothing else.",
        },
    ...
    return numbers.length >= 60 ? numbers.slice(0, 64) : null;
  } catch {
    return null;
  }
}
